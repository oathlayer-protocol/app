...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      // Fetch BreachWarning events
      const warningLogs = await publicClient.getLogs({
        address: SLA_CONTRACT_ADDRESS,
      ...
      // Fetch SLABreached events
      const breachLogs = await publicClient.getLogs({
        address: SLA_CONTRACT_ADDRESS,
  ...
  // Real-time BreachWarning events
  useWatchContractEvent({
    address: SLA_CONTRACT_ADDRESS,
    abi: SLA_ABI,
    eventName: "BreachWarning",
    onLogs(logs) {
      const newWarnings = logs.map((log: Log) => {
        const args = (log as unknown as { args: { slaId: bigint; riskScore: bigint; prediction: string } }).args;
        return {
          slaId: args.slaId,
      ...
      setBreachWarnings(prev => [...newWarnings, ...prev]);
    },
    poll: true,
    pollingInterval: 5_000,
  });

  // Real-time SLABreached events
  useWatchContractEvent({
    address: SLA_CONTRACT_ADDRESS,
    abi: SLA_ABI,
    eventName: "SLABreached",
    onLogs(logs) {
      const newBreaches = logs.map((log: Log) => {
        const args = (log as unknown as { args: { slaId: bigint; provider: string; uptimeBps: bigint; penaltyAmount: bigint } }).args;
        return {
          slaId: args.slaId,
          provider: args.provider,
          uptimeBps: args.uptimeBps,
          penaltyAmount: args.penaltyAmount,
          blockNumber: log.blockNumber!,
          transactionHash: log.transactionHash!,
        };
      });
      setBreachEvents(prev => [...newBreaches, ...prev]);
    },
    poll: true,
    pollingInterval: 5_000,
  });
  ...
  // Build risk score map from latest warnings per SLA
  const latestRiskScores = new Map<number, number>();
  for (const w of breachWarnings) {
    const slaId = Number(w.slaId);
    if (!latestRiskScores.has(slaId)) {
      latestRiskScores.set(slaId, Number(w.riskScore));
    }
  }
